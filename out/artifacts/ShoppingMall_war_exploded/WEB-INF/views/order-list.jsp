...
<div class="page-container">
    ...
    <%--</div>--%>
    <%--<div class="cl pd-5 bg-1 bk-gray mt-20"> <a href="javascript:;" onclick="datadel()" class="btn btn-danger radius"><i class="Hui-iconfont">&#xe6e2;</i> 批量删除</a> <a href="javascript:;" onclick="member_add('添加用户','member-add.html','','510')" class="btn btn-primary radius"><i class="Hui-iconfont">&#xe600;</i> 添加用户</a></span> <span class="r">--%><span class="r">共有数据：<strong>${order.size()}</strong> 条</span> </div>
<div class="mt-20">
    <table class="table table-border table-bordered table-hover table-bg table-sort">
        ...
        <c:forEach var="order" items="${order}" varStatus="status">
            <tr class="text-c">
                ...
                <td class="text-l">${order.order_address}</td>
                <td>${order.order_generatedTime}</td>
                <td class="td-status"><span class="label label-success radius"><c:if test="${order.order_status==0}">待发货</c:if>
                <c:if test="${order.order_status==1}">待收货</c:if><c:if test="${order.order_status==2}">待评价</c:if><c:if test="${order.order_status==3}">已完成</c:if></c></span></td>
                <td class="td-manage">
...
<script type="text/javascript">
    ...
    function isSureToDelete(a) {
        ...
        if(confirm("确定要删除吗?此操作不可恢复!")){
            window.location.href="/admin/delete?status=3&&index="+index;
        }
        else{
            return false;
        }
